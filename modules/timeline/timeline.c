...
int FindPizzas() {
    ...
    free(path_to_right_pizza->path);
    free(path_to_right_pizza);
    free(path_to_left_pizza->path);
    free(path_to_left_pizza);
...
void DeliverPizzas(DeliverySequence *cur_sequence) {
    ...
    if (cur_sequence->order_combo[deliver1] != NULL) {
        printf("%s, %d\n", cur_sequence->order_combo[deliver1]->delivery_house->name, cur_sequence->order_combo[deliver1]->order_time);
    }
