...
int get_block_size(int distance) {
    int threshold = 10;
    int no_block_height = 328; // From experimental values
    int small_height = no_block_height - 60;
    int medium_height = no_block_height - 90;
    int large_height = no_block_height - 120;

    int ret = 0;

    if (fabs(distance - no_block_height) < threshold) {
        ret = 0;
    }
...
int main()
{
    ...
    while(1) {
        ...
        block_size = get_block_size(value); // Get block size from sharp sensor distance (in mm)
